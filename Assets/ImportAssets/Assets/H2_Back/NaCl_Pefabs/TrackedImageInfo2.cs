...
public class TrackedImageInfo2 : MonoBehaviour
{
    [SerializeField]
    private ARTrackedImageManager m_TrackedImageManager;

    [SerializeField]
    private GameObject modelForNa; // H₂ prefab
    [SerializeField]
    private GameObject modelForCl;  // O prefab
    [SerializeField]
    private GameObject modelForNaCl;   // H₂O prefab
    ...
    private void SpawnModelForImage(ARTrackedImage trackedImage)
    {
        ...
        if (trackedImage.referenceImage.name == "Na")
        {
            modelToSpawn = Instantiate(modelForNa, trackedImage.transform.position, Quaternion.identity);
        }
        else if (trackedImage.referenceImage.name == "Cl")
        {
            modelToSpawn = Instantiate(modelForCl, trackedImage.transform.position, Quaternion.identity);
        }
    ...
    private void AddCollisionComponents(GameObject obj)
    {
        ...
        if (!obj.GetComponent<ObjectCollisionHandler>())
        {
            ...
            collisionHandler.resultingPrefab = modelForNaCl;
        }
    }
}
