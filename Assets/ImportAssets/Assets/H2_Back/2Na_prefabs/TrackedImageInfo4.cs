...
public class TrackedImageInfo4 : MonoBehaviour
{
    [SerializeField]
    private ARTrackedImageManager m_TrackedImageManager;

    [SerializeField]
    private GameObject modelFor2Na; // H₂ prefab
    [SerializeField]
    private GameObject modelFor2H2O;  // O prefab
    [SerializeField]
    private GameObject modelFor2NaOH;   // H₂O prefab
    ...
    private void SpawnModelForImage(ARTrackedImage trackedImage)
    {
        ...
        if (trackedImage.referenceImage.name == "2Na")
        {
            modelToSpawn = Instantiate(modelFor2Na, trackedImage.transform.position, Quaternion.identity);
        }
        else if (trackedImage.referenceImage.name == "2H2O")
        {
            modelToSpawn = Instantiate(modelFor2H2O, trackedImage.transform.position, Quaternion.identity);
        }
    ...
    private void AddCollisionComponents(GameObject obj)
    {
        ...
        if (!obj.GetComponent<ObjectCollisionHandler>())
        {
            ...
            collisionHandler.resultingPrefab = modelFor2NaOH;
        }
    }
}
